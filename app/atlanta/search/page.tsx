...
const items: SearchItem[] = [leadStory, ...stories].map((story) => ({ title: story.headline, summary: story.dek, category: story.category, href: `/atlanta/${story.slug}` })).concat(projects.map((project) => ({ title: project.name, summary: `${project.location} · ${project.status} · ${project.detail}`, category: "Project", href: `/atlanta/project/${project.slug}` })));
